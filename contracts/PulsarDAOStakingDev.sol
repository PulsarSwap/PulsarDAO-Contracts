...
contract PulsarDAOStakingDev is ERC20("vePUL", "vePUL"), Ownable, BlockTimeOverridable {
    ...
    function addRewardPUL(uint256 _pulAmount) external {
        ...
        require(blockTime() < cfg.periodFinish, "PulsarDAOStaking: Adding rewards is forbidden");
    ...
    function setPeriod(uint64 _periodStart, uint64 _rewardsDuration) public onlyOwner {
        require(_periodStart >= blockTime(), "PulsarDAOStaking: _periodStart shouldn't be in the past");
        require(_rewardsDuration > 0, "PulsarDAOStaking: Invalid rewards duration");

        Config memory cfg = config;
        require(cfg.periodFinish < blockTime(), "PulsarDAOStaking: The last reward period should be finished before setting a new one");
